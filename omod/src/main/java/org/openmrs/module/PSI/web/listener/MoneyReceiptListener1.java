...
@EnableAsync
@Controller
public class MoneyReceiptListener1 {
	...
	public void sendMoneyReceiptData() throws Exception {
		...
		if (status) {
			try {
				...
				lock.unlock();
				log.error("complete listener MoneyReceipt1 at:" +new Date());
			}
		}
	...
	private synchronized void sendMoneyReceipt() {
		log.error("Entered in send money receipt listener1 " + new Date());
		long timestamp = 0;
		
		PSIDHISMarker getlastTimeStamp = Context.getService(PSIDHISMarkerService.class).findByType("MoneyReceipt1");
		if (getlastTimeStamp == null) {
			PSIDHISMarker psidhisMarker = new PSIDHISMarker();
			psidhisMarker.setType("MoneyReceipt1");
			psidhisMarker.setTimestamp(1615876382553l);
			...
			for (PSIServiceProvision psiServiceProvision : psiServiceProvisions) {
				...
				psiServiceProvision.getUuid();
					...
					try {
						...
						PSIDHISMarker getTimeStamp = Context.getService(PSIDHISMarkerService.class).findByType("MoneyReceipt1");
						log.error("updated at MoneyReceipt1 at"+getTimeStamp+" at "+new Date());
					}
			}
