		    ...
		    <div class="modal-content">
		      ...
		      <div class="modal-body">
		       <div class="row">
				...
				<div class="row">
					<div class="col-lg-12 form-group">
						<label for="invoiceNo">7. Price in Package:</label> 
						<input type="number" min="1" step="1" oninput="this.value = Math.abs(this.value)" style="height: 39px;" class="form-control" id="priceinPackage">
						<span id="priceInproductvalidation" class="text-danger"></span>
					</div>
				</div>
		      </div>
		      ...
		      <div class="modal-body">
		       <div class="row">
				...
				<div class="row">
					<div class="col-lg-12 form-group">
						<label for="invoiceNo">7. Price in Package:</label> 
						<input type="number" min="1" step="1" oninput="this.value = Math.abs(this.value)" style="height: 39px;" class="form-control" id="servicePriceinPackage">
						<span id="servicePriceinPackagevalidation" class="text-danger"></span>
					</div>
				</div>
		      </div>
		      <div class="modal-footer">
...
function appendRowForProductInTable() {
	...
	jQuery("#productQuantityvalidation").html("");
	if(jQuery("#priceinPackage").val() == "") {
		jQuery("#priceInproductvalidation").html("<strong>Please fill out this field</strong>");
		return;
	}
	...
	jQuery("#trainingList tbody").append("<tr id='rec-"+size+"'><td><span class=\"sn\">"+size+"</span>.</td><td>"+productName+"</td><td>"+productId+"</td><td>"+productCode+"</td><td>"+productQuantity+"</td><td>"+unitPrice+"</td><td>"+productAmount+"</td><td>"+priceinPackage+"</td><td><a class=\"btn btn-xs delete-record\" data-id="+size+"><i class=\"fa fa-trash\"></i></a></td></tr>");
...
function appendRowForServiceInTable() {
	...
	jQuery("#serviceQuantityvalidation").html("");
	if(jQuery("#servicePriceinPackage").val() == "") {
		jQuery("#servicePriceinPackagevalidation").html("<strong>Please fill out this field</strong>");
		return;
	}
	jQuery("#servicePriceinPackagevalidation").html("");
	...
	var size = jQuery('#trainingList >tbody >tr').length + 1;
	jQuery("#trainingList tbody").append("<tr id='rec-"+size+"'><td><span class=\"sn\">"+size+"</span>.</td><td>"+serviceName+"</td><td>"+serviceId+"</td><td>"+serviceCode+"</td><td>"+serviceQuantity+"</td><td>"+serviceUnitPrice+"</td><td>"+serviceAmount+"</td><td>"+servicePriceinPackage+"</td><td><a class=\"btn btn-xs delete-record\" data-id="+size+"><i class=\"fa fa-trash\"></i></a></td></tr>");
	calculateTotalPackagePrice();
...
function calculateTotalPackagePrice() {
	...
	jQuery('#trainingList > tbody > tr').each(function(index, tr) {
		...
		var packagePrice = $row.find('td').eq(7).text();
...
function savePackageData() {
	...
	jQuery('#trainingList > tbody > tr').each(function(index, tr) {
				...
				var packageItemPriceInPackage = $row.find('td').eq(7).text();
